...
class LeafNode extends BPlusNode {
  ...
  public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid) throws BPlusTreeException {
      ...
      int keyIdx = 0;
      for(DataBox oneKey : getKeys()){
          if (oneKey.compareTo(key) == 0)
              throw new BPlusTreeException("Duplicated Entries Error");
      ...
      if (numofKeys > 2 * metadata.getOrder()) {
          ...
          List<DataBox> newKeys = keys.subList(midIdx, numofKeys);
  ...
  public static LeafNode fromBytes(BPlusTreeMetadata metadata, int pageNum) {
    ...
    int siblingNum = buf.getInt();
    if (siblingNum == -1)
        rightSibling = Optional.empty();
    else
        rightSibling = Optional.of(siblingNum);
